...
export default function Overs() {
    ...
    return (
        <div className={`p-3 w-full h-full overflow-x-auto overflow-auto ${true ? 'text-[#3b3b3b]' :'text-[#CCCCCC]'} ${true ? '' : 'custom-scrollbar'}`}>
            <div className='transition scale-75 500:scale-[1] flex justify-center'>
                    ...
                    {overs_1.map((over, index) => (
                        <div key={nanoid()} className={`py-2 flex flex-col`}>
                            <div className={`flex 700:gap-x-1`}>
                                <h1 className={`font-robotoSans pr-3 text-[.9rem] 700:text-[1rem] ${true ? 'text-[#3b3b3b]' : 'text-[#797979] text-nowrap'}`}>[ {index + 1} ] &nbsp; <span className='font-robotoSans font-[400] underline underline-offset-4 text-nowrap'>{over[0]}</span></h1>
