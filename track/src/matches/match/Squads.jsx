...
export default function Squads() {
    ...
    return (
        <div className={`p-3 flex flex-col gap-y-2 w-full ${true ? 'text-[#3b3b3b]' : 'text-[#CCCCCC]'} ${true ? '' : 'custom-scrollbar'}`}>
            <div className={`w-full border ${true ? 'border-[#bbbbbb88]' : 'border-[#99999988]'}`}>
                <div className={`p-1 700:p-2 flex gap-x-1 700:gap-x-2 items-center w-full`}>
                    ...
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={true ? '#3b3b3b' : '#CCCCCC'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M6 9l6 6 6-6" />
                        </svg>
                    </div>
                ...
                <div className={`flex flex-col overflow-hidden ${a1 ? 'p-2' : 'h-0'} gap-y-[.35rem] w-full transition duration-75 ease-in-out`}>
                    {team1_batting.map((player, index) => (
                        <div key={index} className={`${true ? (player.status.isBatting ? 'bg-[#f3f3f3]' : '') : (player.status.isBatting ? 'bg-[#2f2f2f]' : '')} flex flex-col w-full border ${true ? 'border-[#bbbbbb80]' : 'border-[#99999960]'}`}>
                            <div className={`flex w-full items-center gap-x-1 px-1 py-1 700:py-2`}>
                                ...
                                <div className='scale-[.8] 700:scale-[1]'>
                                    {player.status.job === 'bowler' ?
                                        <Bowler size='18px' fill={true ? '#3b3b3b' : '#CCCCCC'}/>
                                        :
                                        <Batter batter={player.status.job === 'batter'} size='18px' fill={true ? '#3b3b3b' : '#CCCCCC'}/>
                                    }
                                </div>
                                <h1 className='font-robotoSans font-[100] text-[.8rem] 700:text-[1rem]'>
                                    {player.status.name}&nbsp;&nbsp;
                                    <i className={`font-robotoSans ${!true ? 'text-[#CCCCCC50]' : 'text-black font-[400]'}`}>
                                        {(() => {
                                            let pl = null
                            ...
                            <div className={`overflow-hidden ${expandedPlayers1[index] ? '' : 'h-0'} flex w-full ${true ? 'bg-[#f3f3f3]' : 'bg-[#252525]'}`}>
                                <div className={`flex flex-col w-1/2 items-center border-t border-r ${true ? 'border-[#3b3b3b66]' : 'border-[#999999]'}`}>
                                    ...
                                    <div className={`py-2 px-4 w-full`}>
                                        {(() => {
                                            ...
                                            <div key={index} className={`px-[2px] font-[100] flex justify-between w-full border border-transparent ${true ? 'hover:border-[#3b3b3b90]' : 'hover:border-[#CCCCCC]'}`}>
                                ...
                                <div className={`flex flex-col w-1/2 items-center border-t ${true ? 'border-[#3b3b3b66]' : 'border-[#999999]'}`}>
                ...
                <div className={`flex flex-col overflow-hidden ${a2 ? 'p-2' : 'h-0'} gap-y-[.35rem] w-full transition duration-75 ease-in-out`}>
                    {team2_batting.map((player, index) => (
                        <div key={index} className={`${true ? (player.status.isBatting ? 'bg-[#f3f3f3]' : '') : (player.status.isBatting ? 'bg-[#2f2f2f]' : '')} flex flex-col w-full border ${true ? 'border-[#bbbbbb80]' : 'border-[#99999960]'}`}>
                            ...
                            <div className={`overflow-hidden ${expandedPlayers2[index] ? '' : 'h-0'} flex w-full ${true ? 'bg-[#f3f3f3]' : 'bg-[#252525]'}`}>
                                <div className={`flex flex-col w-1/2 items-center border-t border-r ${true ? 'border-[#3b3b3b66]' : 'border-[#999999]'}`}>
                                    <h1 className='font-robotoSans text-[.8rem] 700:text-[1rem] font-semibold'>Batting</h1>
                                    <div className={`py-2 px-4 w-full`}>
                                        {(() => {
