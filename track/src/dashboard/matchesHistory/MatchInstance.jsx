            ...
            <div className={`flex flex-col justify-center relative w-full`}>
                <div className={`relative flex flex-col`}>
                    <div className={`relative`} ref={optionsRef}>
                        <button
                            disabled={loading}
                            className={`700:px-3 py-2 700:py-3 absolute self-center right-3 ${theme ? '' : 'hover:brightness-[2]'}`}
                            onClick={() => {
                        ...
                        {
                            options ?
                            <ul className={`animate-showUpFast absolute top-[35px] right-[20px] ${theme ? 'bg-[#ffffff]' : 'bg-[#1F1F1F]'} border ${theme ? 'border-[#e5e5e5]' : 'border-[#454545]'} rounded-[4px]`}>
                                <li
                                    className={`text-[.8rem] ${theme ? 'text-[#3b3b3b]' : 'text-[#CCCCCC]'} border-b ${theme ? 'border-b-[#e5e5e5]' : 'border-b-[#454545]'} cursor-pointer`}
                    ...
                    <div className={`flex items-center gap-x-1 700:gap-x-2 pt-1 700:p-1`}>
                        <Link className={`cursor-pointer text-[1.3rem] 700:text-[2rem] ${theme ? 'text-[#3b3b3b]' : 'text-[#CCCCCC]'} font-robotoSans font-[100] underline underline-offset-8 ${theme ? 'hover:text-black' : 'hover:text-white'}`} to={`../match-id/${_id}`}>{name}</Link>
                        {isCompleted ?
                            <svg className='pt-1' xmlns="http://www.w3.org/2000/svg" fill={theme ? 'green' : '#39FF14'} width="40px" height="40px" viewBox="0 0 256 256" id="Flat">
                        ...
                        :
                            <span className={`ml-2 mt-1 flex items-center justify-center relative size-4 700:size-6 rounded-full bg-yellow-600`}>
