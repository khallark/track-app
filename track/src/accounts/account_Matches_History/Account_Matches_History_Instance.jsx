...
export default function Account_Matches_History_Instance({id}) {
    const [data, setData] = useState(null)
    const domain = useSelector(state => state.globals.domain)
    useEffect(() => {
        const getMatch = async () => {
            try {
                const response = await axios.post(`${domain}/auth/matches/${id}`, {
    ...
    return (
        <Link to={`/match/${id}`} target='_blank' className={`transition ease-linear px-5 py-2 flex flex-col border border-[#bdbdbd] justify-center relative w-[94%] 850:w-[97%] rounded-md hover:bg-[#e4e4e4] cursor-pointer`}>
            <div className={`relative flex flex-col text-[#3b3b3b]`}>
                ...
                <h1 className={`pt-2 850:p-1 text-[1.3rem] 850:text-[2rem] font-robotoSans underline underline-offset-8`}>{data?.name}</h1>
                <h1 className={`pt-2 850:p-1 text-[.8rem] 850:text-[1rem] font-robotoSans`}>{data?.location}</h1>
                <h1 className={`py-2 850:p-1 text-[.5rem] 850:text-[.7rem] font-robotoSans`}>{data ? formatDate(data?.date) : ''}</h1>
            </div>
        </Link>
    )
}